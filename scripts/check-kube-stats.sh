...
FROM unnest(
  ARRAY['default','default','kube-system']::text[],
  ARRAY['web-1','web-1','log-1']::text[],
  ARRAY['app','sidecar','log']::text[],
  ARRAY['n1','n1','n2']::text[],
  ARRAY['Running','Running','Running']::text[],
  ARRAY['web','web','logger']::text[],
  ARRAY['Deployment','Deployment','DaemonSet']::text[],
  ARRAY[250,10,30]::bigint[],
  ARRAY[134217728,16777216,33554432]::bigint[],
  ARRAY[2,0,1]::int[],
  ARRAY['{"app":"web"}','{"app":"web"}','{"app":"logger"}']::text[]
...
# aggregate by namespace (latest snapshot): default -> cpu 260, pods 1, containers 2
read -r cpu pods conts <<<"$(psql -c "WITH l AS (SELECT max(time) t FROM kube_container_stats WHERE system_id='$SID') \
  SELECT sum(cpu_millicores), count(DISTINCT pod), count(*) FROM kube_container_stats c, l \
  WHERE c.system_id='$SID' AND c.time=l.t AND namespace='default';" | tr '|' ' ')"
assert "ns=default cpu" "$cpu" "260"
assert "ns=default pods" "$pods" "1"
assert "ns=default containers" "$conts" "2"

# aggregate by workload: Deployment/web -> 260
wl=$(psql -c "WITH l AS (SELECT max(time) t FROM kube_container_stats WHERE system_id='$SID') \
  SELECT sum(cpu_millicores) FROM kube_container_stats c, l \
  WHERE c.system_id='$SID' AND c.time=l.t AND (workload_kind||'/'||workload)='Deployment/web';")
assert "workload Deployment/web cpu" "$wl" "260"

# aggregate by label app=logger -> 30
lbl=$(psql -c "WITH l AS (SELECT max(time) t FROM kube_container_stats WHERE system_id='$SID') \
  SELECT sum(cpu_millicores) FROM kube_container_stats c, l \
...
n=$(psql -c "WITH l AS (SELECT max(time) t FROM kube_container_stats WHERE system_id='$SID') \
  SELECT count(*) FROM kube_container_stats c, l WHERE c.system_id='$SID' AND c.time=l.t AND namespace='default';")
assert "containers ns=default rows" "$n" "2"

# series (per-snapshot sum then per-bucket avg): >=1 bucket, cpu total 290
...
assert "latest snapshot total cpu" "$scpu" "290"
