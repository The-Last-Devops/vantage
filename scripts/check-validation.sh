...
NS=$(curl -s -b "$JAR" "$BASE/api/namespaces" | python3 -c "import sys,json;d=json.load(sys.stdin);print(d[0]['id'] if d else '')")
...
# --- display names ---
exp "reject empty channel name"       400 "$(code POST "/api/namespaces/$NS/channels" '{"name":"   ","kind":"webhook","config":{"url":"https://e.com/x"}}')"
exp "reject control-char channel name" 400 "$(code POST "/api/namespaces/$NS/channels" '{"name":"bad\nname","kind":"webhook","config":{"url":"https://e.com/x"}}')"
exp "reject empty monitor name"       400 "$(code POST "/api/namespaces/$NS/monitors" '{"name":"","kind":"http","target":"https://e.com"}')"
exp "reject http monitor w/o target"  400 "$(code POST "/api/namespaces/$NS/monitors" '{"name":"probe","kind":"http","target":"  "}')"
