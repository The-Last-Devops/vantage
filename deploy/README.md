...
```bash
helm install vantage oci://ghcr.io/the-last-devops/charts/vantage --version 3.0.3 \
...
> `…/vantage-agent` (agent) at that version. `helm show values oci://…/vantage --version 3.0.3`
...
| `image.tag` | `3.0.3` | Pinned release tag. Bump + `helm upgrade` to update; or set a moving tag (`latest`/`main`) with `image.pullPolicy=Always`. |
...
Defaults to the `:latest` image; add `&tag=main` (rolling) or `&tag=3.0.3` (pinned) to
...
no clone needed:
```bash
helm install vantage-agent oci://ghcr.io/the-last-devops/charts/vantage-agent --version 3.0.3 \
  --namespace vantage --create-namespace \
...
| `image.tag` | — | `3.0.3` | Pinned tag; bump + `helm upgrade` (or a moving tag + `pullPolicy=Always`) to update. |
...
Fix: install **3.0.1+** (`--version 3.0.3`, which pins `image.tag: 3.0.1`). Already on 3.0.0?

```bash
helm upgrade vantage oci://ghcr.io/the-last-devops/charts/vantage --version 3.0.3 \
...
(e.g. `:3.0.3`) + `:latest`; `:main` is the rolling build from `main`. The chart pins
`:3.0.3` by default.
