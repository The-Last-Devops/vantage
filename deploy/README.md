...
```bash
helm install vantage oci://ghcr.io/the-last-devops/charts/vantage --version 3.0.7 \
...
> `…/vantage-agent` (agent) at that version. `helm show values oci://…/vantage --version 3.0.7`
...
no clone needed:
```bash
helm install vantage-agent oci://ghcr.io/the-last-devops/charts/vantage-agent --version 3.0.7 \
  --namespace vantage --create-namespace \
...
```bash
helm upgrade vantage oci://ghcr.io/the-last-devops/charts/vantage --version 3.0.7 \
...
(e.g. `:3.0.7`) + `:latest`; `:main` is the rolling build from `main`. The chart pins
