<script setup>
// The dynamic notify-channel config form: a namespace picker (create only), a name,
...
// ChannelFieldInput), and `name`/`nsId` are bound through their own v-models.
...
const nsId = defineModel('nsId', { default: '' })
...
<template>
  <fieldset :disabled="readOnly" class="max-h-[60vh] space-y-4 overflow-auto p-5">
    ...
    <label v-if="!editId" class="block">
      ...
      <UiSelect v-model="nsId" block :options="namespaces.map((n) => ({ value: n.id, label: n.name }))" />
