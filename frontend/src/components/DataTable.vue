  ...
  <div>
    ...
    <div class="overflow-x-auto rounded-b-xl border border-line bg-surface">
      <table class="w-full border-collapse">
        <thead>
          <tr>
            <th v-if="selectable" class="w-8 border-b border-line-strong bg-headbg px-3 py-2">
              <input ref="headCb" type="checkbox" :checked="allSel" @change="toggleAll" class="h-[14px] w-[14px] cursor-pointer align-middle accent-[rgb(var(--accent))]" />
            </th>
            <th v-for="c in columns" :key="c.key" @click="toggleSort(c)"
              class="select-none border-b border-line-strong bg-headbg px-3 py-2 text-[11px] font-semibold text-muted"
        ...
        <tbody>
          ...
          <tr v-for="row in sorted" :key="rowKey(row)"
            ...
            <td v-if="selectable" class="px-3 py-1.5" @click.stop>
              <input type="checkbox" :checked="selected.includes(rowKey(row))" @change="toggleRow(rowKey(row))" class="h-[14px] w-[14px] cursor-pointer align-middle accent-[rgb(var(--accent))]" />
            </td>
            <td v-for="c in columns" :key="c.key" class="px-3 py-1.5 text-xs text-fg"
